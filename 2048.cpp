...
int main()
{
    ...
    delimage(img);
	 rand_map();
		...
		//背景music 
	MUSIC music;
	music.OpenFile("beijing_music.mp3");
	//检查是否打开
	if (music.IsOpen())
	{
		//xyprintf(200, 220, "%s\n", "音乐打开成功");
...
{	
	if((x>673 && x<747)&& (y<260 && y>225) )
	{	
 	up();
 	rand_map();
 	}
 	if((x>675 && x<748)&& (y>337 && y<370) )
	{
	down();
	rand_map();
	}
 	if((x>637 && x<673)&& (y<335 && y>263))
	{ 
 	left();
 	rand_map();
 	}
 	if((x>750 && x<785)&& (y<336 && y>264))
	{ 
 	right();
 	rand_map();
 	}
 	if((x>676 && x<747)&& (y<332 && y>266))
	{ 
